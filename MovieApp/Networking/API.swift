...
class API {
    ...
    func getSearchData(param: Parameters) -> Observable<APIResult<GetMovieResponse>> {
        
        return API.handleDataRequest(dataRequest: APIManager.shared.requestObservable(api: APIRouter.getSearchMovies(param))).map({ (response) -> APIResult<GetMovieResponse> in
            if (response ?? [:]).keys.contains("Error") {
                if (response ?? [:]).keys.contains("IsInternetOff") {
                    if let isInternetOff = response!["IsInternetOff"] as? Bool {
                        if isInternetOff {
